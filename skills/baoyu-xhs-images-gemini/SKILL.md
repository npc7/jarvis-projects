---
name: baoyu-xhs-images-gemini
description: Generates Xiaohongshu (Little Red Book) infographic series with 10 visual styles and 8 layouts using Google Gemini API. Breaks content into 1-10 cartoon-style images optimized for XHS engagement. Use when user mentions "小红书图片", "XHS images", "RedNote infographics", "小红书种草", or wants social media infographics for Chinese platforms.
---

# Xiaohongshu Infographic Series Generator (Gemini API Version)

Break down complex content into eye-catching infographic series for Xiaohongshu with multiple style options. This version uses Google Gemini API for image generation.
...
2. Generate image using Gemini API:
   ```bash
   # Set API key from environment variable
   export GEMINI_API_KEY="your_api_key_here"
   
   # Generate image using the Python script
   python3 scripts/generate_image.py \
     --prompt "prompt content from prompt file" \
...
This skill uses Google Gemini API for image generation. To use it:
1. Set the `GEMINI_API_KEY` environment variable in your `.env` file
2. The script will automatically try `gemini-3-pro-image-preview` first, and fall back to `gemini-2.5-flash-image` if the first model fails

**Session Management**:
The script includes session management to ensure visual consistency across all images in a series.
...
2. Install required Python packages:
   ...
   ```
3. Create a `.env` file in the skill directory with your Gemini API key:
   ```
...
- Gemini API has rate limits and usage quotas. Check Google AI Studio for details.
